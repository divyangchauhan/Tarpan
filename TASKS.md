...
| P6-06 | Rate limiting: NestJS throttler guard on auth + upload endpoints         | ✅     | PR #15 — `@nestjs/throttler` wired globally; `register` 5/60s, `login` 10/60s, `refresh` 20/60s, `initiateUpload` + `create generated-document` 20/60s; `@SkipThrottle()` on internal Lambda callback endpoints |
| P6-07 | Health check endpoints + ALB health check hardening                      | ✅     | PR #15 — `@nestjs/terminus` replaces basic health endpoint; `GET /api/v1/health` (versioned) checks TypeORM DB ping + heap memory (300 MB); ALB `healthyHttpCodes` tightened from `200,404` to `200`; `@SkipThrottle()` on the health route |
